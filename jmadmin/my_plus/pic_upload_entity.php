...
//	$gcmc=$_POST["gcmc"];
	...
	if ($result -> num_rows > 0) {
		if($lx=='cjzp'){
			$sqli = "update 实体自检  set  检测前照片='".$filenames."',场景照片说明='".$Text1."' where 时间戳='".$mchen."' ";
		}
		if($lx=='ypzp'){
			$sqli = "update 实体自检  set  检测实施过程照片='".$filenames."',检测实施过程照片说明='".$Text2."' where 时间戳='".$mchen."' ";
		}
		if($lx=='sbzp'){
			$sqli = "update 实体自检  set  检测设备照片='".$filenames."',检测设备照片说明='".$Text3."'  where 时间戳='".$mchen."' ";
		}
		if($lx=='zczp'){
			$sqli = "update 实体自检   set  自测照片='".$filenames."',自测照片说明='".$Text4."'  where 时间戳='".$mchen."' ";
		}
		if($lx=='clzp'){
			$sqli = "update 实体自检   set  处理照片='".$filenames."',处理照片说明='".$Text5."'  where 时间戳='".$mchen."' ";
		}
	}
