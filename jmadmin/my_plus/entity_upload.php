...
for ($i= 0;$i< $nub3; $i++){
	...
	if($lx=='insert'){
		...
		$sql = "insert into 实体检测(时间戳,工程名称,工程时间戳,检测类型,检测部位,数量,检测人,检测日期,备注,检测单位,工程单状态,监理操作单位,检测操作单位) values ('$mchen','$pj_name','$pj_timestamp','$Info[0]',
'$Info[1]','$Info[2]','$Info[3]','$Info[4]','$Info[5]','$Info[6]','新建','$Info[7]','$Info[6]')";
	...
	if($lx=='updateForm'){
		...
		$Info = explode("|", $formInfo);
		$sql = "update 实体检测  set 检测类型='$Info[0]',检测部位='$Info[1]',数量='$Info[2]',检测人='$Info[3]',检测日期='$Info[4]',备注='$Info[5]',检测单位='$Info[6]' where 时间戳='$mchen' ";
		$conn->query($sql);
		$conn->close();
	}
	//叠加附件
	if($lx=='update1'){
		$Text1 = $_POST["Text1"];
		$sql = "update 实体检测  set 场景照片说明='".$Text1."',检测前照片=concat(检测前照片,'".$filenames1."') where 时间戳='$mchen' ";
		$conn->query($sql);
		$conn->close();
	}
	if($lx=='update2'){
		$Text2 = $_POST["Text2"];
		$sql = "update 实体检测  set 检测实施过程照片说明='$Text2',检测实施过程照片=concat(检测实施过程照片,'".$filenames2."') where 时间戳='$mchen' ";
		$conn->query($sql);
		$conn->close();
	}
	if($lx=='update3'){
		$Text3 = $_POST["Text3"];
		$sql = "update 实体检测  set 检测设备照片说明='$Text3',检测设备照片=concat(检测设备照片,'".$filenames3."') where 时间戳='$mchen' ";
		$conn->query($sql);
		$conn->close();
	}
	if($lx=='step1'){
		...
		$Text2 = $_POST["Text2"];
		$Text3 = $_POST["Text3"];
      	$sqli = " update 实体检测  set 检测前照片='$filenames1',检测实施过程照片='$filenames2',检测设备照片='$filenames3',场景照片说明='$Text1',检测实施过程照片说明='$Text2',检测设备照片说明='$Text3' where 时间戳='$mchen' ";
	}
	...
	if ($result -> num_rows > 0) {
		if($lx=='syzp'){
			...
			$str = explode("|",$dataAll);
//			提交见证时的状态更改和其他数据上传
			$sqli = "update 实体检测  set 送样日期='".$str[0]."',收样日期='".$str[1]."',工程单状态 = '已确认',送样单位='".$str[2]."',见证单位='".$str[3]."',收样单位='".$str[4]."',见证人='".$str[5]."',实测照片='".$filenames1."',实测照片说明='".$Text4."',检测操作单位='".$operation_unit."' where id='".$id."'";
		}
		if($lx=='clzp'){
			...
			$report=$_POST["report"];
			$id=$_POST["id"];
			$sqli = "update 实体检测  set  不合格报告 = '".$tresult."',检测报告编号 = '".$testNum."',报告照片说明 = '".$report."',工程单状态 = '不合格',不合格报表='".$filenames1."' where id='".$id."'";
		}
		if($lx=='wtjc'){
			$tresult=$_POST["tresult"];
			$id=$_POST["id"];
			...
			$sqli = "update 实体检测  set  退场记录 = '".$tresult."',工程单状态 = '待审批',处理照片='".$filenames1."',处理照片说明='".$Text6."',处理类型='".$process_type."'  where id='".$id."'";
		}
	}
