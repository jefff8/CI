	...
	if($flag == "获取信息" ){
		...
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()){
				...
				$return_data['规格']=$row['规格'];
				$return_data['数量']=$row['数量'];
				$return_data['生产厂家']=$row['生产厂家'];
				$return_data['取样人']=$row['取样人'];
				$return_data['进厂日期']=$row['进厂日期'];
				$return_data['取样日期']=$row['取样日期'];
				$return_data['合格证编号']=$row['合格证编号'];
				$return_data['使用部位']=$row['使用部位'];
				$return_data['经销商单位']=$row['经销商单位'];
		...
		$project_id = $_POST['gcid'];
		require("../conn.php");
		$sql = "select 时间戳,工程单状态,检测前照片,检测实施过程照片,检测设备照片,自测照片,处理照片 from 实体自检   where id='$project_id' and 时间戳='$timestamp'";
		$result = $conn->query($sql);
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()){
