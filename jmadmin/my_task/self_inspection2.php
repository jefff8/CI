<?php
	...
	switch($flag){
		case "获取状态":
			$sql = "select 工程单状态,工程名称,工程时间戳  from 实体自检  where  id = '$ulId' " ;
		...
		case "单位名称":
			...
			break;
		case "删除":
			$sql = "delete from 实体自检  where id='$ulId'";
			$result = $conn->query($sql);
			if($result){
				$return['result'] = "删除成功！";
			}else{
			...
			break;
		case "准备材料":
			$sql = "update 实体自检 set 工程单状态='准备材料' where id='$ulId'";
			$result = $conn->query($sql);
			if($result){
				$return['result'] = "成功";
			}else{
			...
			break;
		case "提交见证":
			$sql = "update 实体自检 set 工程单状态='未检测' where id='$ulId'";
			$result = $conn->query($sql);
			if($result){
				$return['result'] = "提交成功";
			}else{
			...
			break;
		case "撤销取样":
			$sql = "update 实体自检 set 工程单状态='新增' where id='$ulId'";
			$result = $conn->query($sql);
			if($result){
				$return['result'] = "撤销成功";
			}else{
			...
			break;
		case "合格":
			$sql = "update 实体自检 set 工程单状态='合格' where id='$ulId'";
			$result = $conn->query($sql);
			if($result){
				$return['result'] = "操作成功";
			}else{
